...
SUBMIT_FILE = os.path.join(BASE_DIR, "submit_linear.csv")

# 模型路径
# 1. 原始预训练 BERT 路径 (用于初始化模型结构)
PRETRAINED_BERT_PATH = os.path.join(BASE_DIR, "bert_small_4096_final")
# 2. 微调后的权重路径 (用于加载训练好的参数)
FT_MODEL_PATH = os.path.join(BASE_DIR, "output_ft_linear/final_model/model.safetensors")
# 备用路径 (如果 safetensors 不存在，尝试 bin)
FT_MODEL_PATH_BIN = os.path.join(BASE_DIR, "output_ft_linear/final_model/pytorch_model.bin")

# 超参数
# 🚀 多卡并行配置
os.environ["CUDA_VISIBLE_DEVICES"] = "0,3,5" # 指定空闲显卡
BATCH_SIZE = 64 * 3 # 3张卡并行，Batch Size 翻 3 倍
SEQ_LEN = 4096
NUM_LABELS = 14
...
def main():
    ...
    if os.path.exists(FT_MODEL_PATH):
        print(f"Loading safetensors from {FT_MODEL_PATH}...")
        state_dict = load_file(FT_MODEL_PATH)
    elif os.path.exists(FT_MODEL_PATH_BIN):
        print(f"Loading pytorch_model.bin from {FT_MODEL_PATH_BIN}...")
        state_dict = torch.load(FT_MODEL_PATH_BIN, map_location="cpu")
    else:
        raise FileNotFoundError(f"Model weights not found at {FT_MODEL_PATH} or {FT_MODEL_PATH_BIN}")
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Inferencing"):
            ...
            # 保存 logits 而不是 argmax
            all_preds.append(logits.cpu().numpy())
            
    # 6. 保存结果
    print("Concatenating logits...")
    all_logits = np.concatenate(all_preds, axis=0) # (N, 14)
    
    print(f"Saving logits to {BASE_DIR}/submit_linear_logits.npy ...")
    np.save(os.path.join(BASE_DIR, "submit_linear_logits.npy"), all_logits)
    
    # 同时生成一份普通的 csv 方便查看
    preds = np.argmax(all_logits, axis=1)
    print(f"Saving results to {SUBMIT_FILE}...")
    submit_df = pd.DataFrame({"label": preds})
    submit_df.to_csv(SUBMIT_FILE, index=False)
    
    print("🎉 Done! Check submit_linear.csv")
