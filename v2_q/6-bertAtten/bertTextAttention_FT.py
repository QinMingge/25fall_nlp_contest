...
if DEBUG_MODE:
    OUTPUT_DIR = "output_ft_attention_debug"
else:
    OUTPUT_DIR = "output_ft_attention"
...
if DEBUG_MODE:
    ...
    NUM_EPOCHS = 10
    # 🚀 Attention 模型比 Linear 稍重，但比 RNN 轻得多
    # 尝试 32，如果 OOM 可以降到 16
    BATCH_SIZE = 32 
...
def main():
    ...
    # 3. 初始化模型
    if is_main_process:
        logger.info("Initializing BertTextAttention model...")
    ...
    training_args = TrainingArguments(
        ...
        metric_for_best_model="f1_macro", # 关注 F1
        greater_is_better=True,
        
        # 混合精度与加速
        fp16=False,
        bf16=True, # A40 支持 BF16，Attention 结构稳定，可以用 BF16
        dataloader_num_workers=8, # 提高数据加载速度
        
        # DDP 配置
        ddp_find_unused_parameters=False, # 关键：设为 False 提高速度
