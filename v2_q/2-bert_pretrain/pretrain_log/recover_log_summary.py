...
def recover_log_summary(checkpoint_dir, output_log_file):
    ...
    # 4. Filter log history for 0.5 epoch intervals
    if not log_history:
        return

    max_epoch = log_history[-1]['epoch']
    targets = []
    current_target = 0.5
    while current_target <= max_epoch:
        targets.append(current_target)
        current_target += 0.5
    
    summary_entries = []
    for t in targets:
        # Find entry with min abs(epoch - t)
        # We can optimize this since log_history is sorted by epoch, but linear scan is fast enough here
        best_entry = min(log_history, key=lambda x: abs(x['epoch'] - t))
        summary_entries.append(best_entry)
    ...
    print(f"Successfully generated summary log to {output_log_file}")

if __name__ == "__main__":
    checkpoint_dir = "/data/jinda/qinmingge/BERT/25fallnewsclassify/output_small_4096"
    output_log_file = "recovered_pretrain_summary.log"
    recover_log_summary(checkpoint_dir, output_log_file)
